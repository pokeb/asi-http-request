...
// These methods alter backgroundTask and are only useful on newer iphones.
- (void)allowRequestAsBackgroundTask;
- (void)removeRequestFromBackground;
...
- (void)main
{
	@try {
		...
		[[self cancelledLock] lock];
		[self allowRequestAsBackgroundTask];
...
- (void)markAsFinished
{
    ...
    [self removeRequestFromBackground];
	CFRelease(self);
}
